...
3. Review the script output for the normalized id, paper folder path, layout markdown path, and bilingual PDF path.
4. Read `layout.md` before writing any interpretation.
...
During import, the script uses a temporary `paper.pdf` only long enough to extract `layout.md` and render `bilingual.pdf`. After `bilingual.pdf` is created successfully, it deletes `paper.pdf`.
